...
public:
	Plane(){};
	Plane(const Vertex &corner, const Vertex &left, const Vertex &right, const QString &textureName="GROUND1_6");
	Vertex corner;
	Vertex left;
	Vertex right;
...
public:
	...
	QValueList<Plane> planes;
	QString toString();
	void move(const int x, const int y, const int z=0);
	void paint(const Texture &texture);
	QValueList<Block> subtract(Block &block);
	
	QString name;
};

class Blocks : public QValueList<Block> {
	public:
			Blocks(){};	
};
