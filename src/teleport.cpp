...
Teleport::Teleport(const Vertex & start, const Vertex & end,
                   const QString & name, int angle):Object(name)
{
    ...
    // The box users see
    Room room(teleportWidth + teleportBorder, teleportHeight + teleportBorder,
              teleportDepth + teleportBorder,
              QString("Teleport %1 walls ").arg(name), teleportBorder);
    room.move(start.x, start.y, start.z + 8);

    QList < Block >::iterator it;
    for (int i = 0; i < room.size(); ++i)
        if (room.at(i).name.contains("rightWall"))
            room.takeAt(i);

    for (int i = 0; i < room.size(); ++i)
        if (room.at(i).name.contains("leftWall"))
            room.takeAt(i);
    ...
    Block box =
        makeBox(x() - 32, y() - 32, z(), 96, 128, 4, "LGMETAL2",
                QString("Teleport Floor %1").arg(name));
    ...
    box =
        makeBox(startJump.x, startJump.y, startJump.z, boxWidth, boxHeight,
                teleportDepth, "*TELEPORT", QString("Teleport %1").arg(name));
    append(box);
