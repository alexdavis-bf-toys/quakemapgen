    ...
    for (uint r = 0; r < width; r++) {
        ...
        if (previousRow[r] & RIGHT && !(row[r] & RIGHT))
            ...
            if (isFirst)
                ...
                if (!isFirst && !(previousRow[r] & RIGHT))
                    printf("|");
                else if (previousRow[r] & DOWN)
                    printf(" ");
                else
                    printf("_");
            }
...
void makeRow(bool isLast)
{
    ...
    for (uint i = 0; i < width; i++) {
        ...
        // Braid
        if (!isLast) {
            if ((row[i] == LEFT) || (row[i] == EMPTY) || (row[i] == RIGHT)) {
                row[i] |= DOWN;
            }
        }
        ...
        if (i > 0) {
            ...
            // no up dead ends
            if ((row[i] == UP) || (row[i] == DOWN)) {
                row[i] |= LEFT;
                row[i - 1] |= RIGHT;
                unionSet(set[i], set[i - 1]);
            }

            if ((row[i] & UP) && (row[i] & LEFT) && (row[i] & RIGHT)) {
                row[i] |= DOWN;
            }

            /*if( row[i] & UP && !(row[i] & LEFT) &&
               (previousRow[i] & LEFT) && (previousRow[i-1] & DOWN)){
               row[i]   |= LEFT;
               row[i-1] |= RIGHT;
               unionSet( set[i], set[i-1] );
               } */
