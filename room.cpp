...
width(width), height(height), depth(depth)
{
    ...
    skin = "CITY2_6";
    Block leftWall =
        makeBox(x, y, z, /*width */ thickness, height, depth, skin,
                QString("Room %1, block leftWall").arg(name));
    append(leftWall);

    Block rightWall =
        makeBox(x + width, y, z, /*width */ thickness, height, depth, skin,
                QString("Room %1, block rightWall").arg(name));
    append(rightWall);

    Block frontWall =
        makeBox(x, y, z, width, /*height */ thickness, depth, skin,
                QString("Room %1, block frontWall").arg(name));
    append(frontWall);

    Block backWall =
        makeBox(x, y + height - thickness, z, width, /*height */ thickness,
                depth, skin, QString("Room %1, block backWall").arg(name));
    append(backWall);

    skin = "ceiling1_3";
    Block celeine =
        makeBox(x, y, z + depth, width, height, /*depth */ thickness, skin,
                QString("Room %1, block celing").arg(name));
    append(celeine);

    skin = "AFLOOR1_8";
    Block floor =
        makeBox(x, y, z, width, height, /*depth */ thickness, skin,
                QString("Room %1, block floor").arg(name));
    append(floor);
}
...
void Room::light()
{
    ...
    for (int w = 1; w < (width / 64) - 1; w++) {
        for (int h = 1; h < (height / 64) - 1; h++) {
            ...
            if (r == 1) {
                Entity light("light");
                light.setOrigin(x() + w * 64 + 32, y() + h * 64 + 32,
                                z() + 32 + pull * 10);
    ...
    for (int w = 1; w < (width / 64) - 1; w++) {
        for (int h = 1; h < (height / 64) - 1; h++) {
            ...
            if (r == 1) {
                ...
                itemEntity.setOrigin(x() + w * 64 + 32, y() + h * 64 + 32,
                                     z() + 64);
