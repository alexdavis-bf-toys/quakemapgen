...
ImageTemplateRoom::ImageTemplateRoom(const QString & fileName,
                                     const QString & name):Object(name)
{
    ...
    Block box =
        makeBox(0, 0, 0, 64 * imageFile.width(), 64 * imageFile.height(), 6,
                "AFLOOR1_4", QString("Floor %1").arg(objectName));
    append(box);
}

int ImageTemplateRoom::getHeight(const int w, const int h) const const
{
    // Try hor
    uint hor = 0;
    for (int i = w; i < imageFile.width(); i++)
        if (imageFile.pixel(i, h) == WALL)
            break;
        else
            hor++;
    for (int i = w; i > 0; i--)
...
void ImageTemplateRoom::draw(QRgb p, int w, int h)
{
    ...
    if (p == ZONE1)
        floorChoice = "afloor3_1";
    if (p == ZONE2)
        floorChoice = "afloor1_4";
    if (p == ZONE3)
        floorChoice = "afloor1_8";
    if (p == ZONE4)
        floorChoice = "afloor1_3";
    ...
    Block box =
        makeBox(w * 64, h * 64, 128 + pull * 20, 64, 64, 256, celingTexture,
                QString("Celine %1").arg(objectName));
    append(box);
    ...
    if (h > 0 && h < imageFile.height() - 1 &&
        ...
        if (imageFile.pixel(w, h - 1) == WALL
            && imageFile.pixel(w - 1, h) == WALL) {
            drawTriangle(w * 64, h * 64, 0, 64, 64, 512, wallChoice, true,
                         false);
            return;
        }
        if (imageFile.pixel(w, h + 1) == WALL
        ...
        if (imageFile.pixel(w, h - 1) == WALL
            && imageFile.pixel(w + 1, h) == WALL) {
            drawTriangle(w * 64, h * 64, 0, 64, 64, 512, wallChoice, false,
                         false);
            return;
        }
        if (imageFile.pixel(w, h + 1) == WALL
    ...
    if (p == WALL) {
        box =
            makeBox(w * 64, h * 64, 0, 64, 64, 512, wallChoice,
                    QString("Wall %1").arg(objectName));
        append(box);
    ...
    if (random(10) == 1 && totalTeleports <= 3) {
        ...
        static QList < Vertex > startPoints;
        if (p != WALL &&
            imageFile.pixel(w, h + 1) != WALL
            && imageFile.pixel(w, h - 1) != WALL
            && imageFile.pixel(w + 1, h) != WALL
            && imageFile.pixel(w - 1, h) != WALL
            && imageFile.pixel(w + 1, h + 1) != WALL
            && imageFile.pixel(w - 1, h + 1) != WALL
            && imageFile.pixel(w + 1, h - 1) != WALL
            && imageFile.pixel(w - 1, h - 1) != WALL) {
        ...
        if (startPoints.count() > 10) {
            ...
            Teleport teleport(startPoints.back(), end,
                              QString("%1_%1").arg(objectName).
                              arg(teleportCount), 180);
            teleportCount++;
            add(teleport);
            end = startPoints.back();
            end.move(-16, 32, 16);
            Teleport teleport2(startPoints.front(), end,
                               QString("%1_%1").arg(objectName).
                               arg(teleportCount), 180);
            add(teleport2);
    ...
    if (g < 5) {
        ...
        if (itemCount[weapon] < itemLimit[weapon]) {
            ...
            qDebug("itemCount: %d, itemLimit:%d", itemCount[weapon],
                   itemLimit[weapon]);
    }
    ...
    if (g == 2) {
        ...
        if (gods < 2) {
            ...
            switch (random(2)) {
            case 1:
                weapon = "item_artifact_invisibility";
                break;
            case 2:
                weapon = "item_artifact_super_damage";
                break;
...
void ImageTemplateRoom::drawTriangle(int x, int y, int z, int width,
                                     int height, int depth,
                                     const QString & skin, bool hor, bool top)
{
    ...
    if (hor && !top || !hor && top) {
        //printf("// front left\n");
        Plane frontLeft(Vertex(x, 0, 0), Vertex(x, 1, 0), Vertex(x, 0, 1),
                        skin);
    ...
    if (hor && !top || !hor && !top) {
        //printf("// front right\n");
        Plane frontRight(Vertex(0, y, 0), Vertex(0, y, 1), Vertex(1, y, 0),
                         skin);
    ...
    if (hor) {
        if (top) {
            ...
            Plane diag(Vertex(x, y + height, 0), Vertex(x, y + height, 1),
                       Vertex(x + width, y, 0), skin);
            box.planes.append(diag);
        }
        else {
            //printf("( %d %d 0 ) ( %d %d 0 ) ( %d %d 1 ) %s 0 0 0 1.0 1.0\n", x, y+height,   x+width, y, x, y+height,  skin.latin1());
            Plane diag(Vertex(x, y + height, 0), Vertex(x + width, y, 0),
                       Vertex(x, y + height, 1), skin);
            box.planes.append(diag);
        }
    }
    else {
        if (top) {
            ...
            Plane diag(Vertex(x, y, 0), Vertex(x, y, 1),
                       Vertex(x + width, y + height, 0), skin);
            box.planes.append(diag);
        }
        else {
            //printf("( %d %d 0 ) ( %d %d 0 ) ( %d %d 1 ) %s 0 0 0 1.0 1.0\n",
            //      x, y,    x+width, y+height, x, y, skin.latin1());
            Plane diag(Vertex(x, y, 0), Vertex(x + width, y + height, 0),
                       Vertex(x, y, 1), skin);
            box.planes.append(diag);
        }
    }
    ...
    Plane bottom(Vertex(0, 0, z + depth), Vertex(0, 1, z + depth),
